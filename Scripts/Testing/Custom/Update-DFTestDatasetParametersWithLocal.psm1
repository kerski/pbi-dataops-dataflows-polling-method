<#
    ...
    $UpdateResult = Update-DFTestDatasetParametersWithLocal -WorkspaceId $_WorkspaceObj.Id.Guid `
                                                ...
                                                -RunId $_RunID  
#>
Function Update-DFTestDatasetParametersWithLocal {
    [CmdletBinding()]
    Param(
              [Parameter(Position = 0, Mandatory = $true)][String]$WorkspaceId, 
              [Parameter(Position = 1, Mandatory = $true)][String]$DatasetId,
              [Parameter(Position = 2, Mandatory = $true)][String]$DataflowWorkspaceId,
              [Parameter(Position = 3, Mandatory = $true)][String]$DataflowId,
              [Parameter(Position = 4, Mandatory = $false)][String]$RunId="Not Ready"                          
    )
    Process {
          Try {
 ...
 Export-ModuleMember -Function Update-DFTestDatasetParametersWithLocal